...
export class ProjectileVisualSystem {
  ...
  private drawBasicProjectile(ctx: CanvasRenderingContext2D, options: ProjectileRenderOptions) {
    ...
    ctx.beginPath()
    ctx.arc(0, 0, options.size, 0, Math.PI * 2)
    ctx.fill()
  ...
  private drawPiercingProjectile(ctx: CanvasRenderingContext2D, options: ProjectileRenderOptions) {
    const gradient = ctx.createLinearGradient(-options.size, 0, options.size, 0)
    gradient.addColorStop(0, options.color)
    gradient.addColorStop(1, '#ffffff')
    
    ctx.fillStyle = gradient
    ctx.beginPath()
    ctx.ellipse(0, 0, options.size * 1.5, options.size * 0.7, 0, 0, Math.PI * 2)
    ctx.fill()
  }
  ...
  private drawExplosiveProjectile(ctx: CanvasRenderingContext2D, options: ProjectileRenderOptions) {
    ctx.fillStyle = '#ff4400'
    ctx.beginPath()
    ctx.arc(0, 0, options.size * 1.2, 0, Math.PI * 2)
    ctx.fill()

    // 导火索效果
    ctx.fillStyle = '#ffaa00'
    ctx.beginPath()
    ctx.arc(options.size * 0.5, 0, 2, 0, Math.PI * 2)
    ctx.fill()
  }
  ...
  private drawArrowProjectile(ctx: CanvasRenderingContext2D, options: ProjectileRenderOptions) {
    // 箭杆
    ctx.fillStyle = '#8B4513'
    ctx.fillRect(-options.size * 2, -1, options.size * 3, 2)
    
    // 箭头
    ctx.fillStyle = '#C0C0C0'
    ctx.beginPath()
    ctx.moveTo(options.size, 0)
    ctx.lineTo(options.size * 0.5, -options.size * 0.5)
    ctx.lineTo(options.size * 0.5, options.size * 0.5)
    ctx.closePath()
    ctx.fill()
  }
  ...
  private drawGrenadeProjectile(ctx: CanvasRenderingContext2D, options: ProjectileRenderOptions) {
    ...
    for (let i = 0; i < 4; i++) {
      ctx.beginPath()
      ctx.arc(0, 0, options.size * (0.3 + i * 0.2), 0, Math.PI * 2)
      ctx.stroke()
    }
  }
  ...
  private drawLaserProjectile(ctx: CanvasRenderingContext2D, options: ProjectileRenderOptions) {
    // 连续激光束 - 长条形而不是点状
    const beamLength = options.size * 8  // 更长的激光束
    const beamWidth = 3
    ...
    ctx.shadowBlur = 5
    ctx.beginPath()
    ctx.arc(-beamLength/2, 0, 2, 0, Math.PI * 2)
    ctx.arc(beamLength/2, 0, 2, 0, Math.PI * 2)
    ctx.fill()
  ...
  private drawMagicProjectile(ctx: CanvasRenderingContext2D, options: ProjectileRenderOptions) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const x = Math.cos(angle) * options.size
      const y = Math.sin(angle) * options.size
      if (i === 0) {
        ctx.moveTo(x, y)
      } else {
        ctx.lineTo(x, y)
